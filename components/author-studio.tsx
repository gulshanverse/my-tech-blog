...
import { ContentTypeNavigation, ProjectManager, TopicManager, type ContentType } from "@/components/content-type-managers";
...
import { normalizeDraftInput, type ArticleDraftInput } from "@/lib/article-draft";
...
type Props = { initialView?: "dashboard" | "editor"; initialPath?: string; initialPathError?: string; initialContentType?: ContentType };
...
export function AuthorStudio({ initialView = "dashboard", initialPath = "", initialPathError = "", initialContentType = "blog" }: Props) {
  ...
  const [editorLoading, setEditorLoading] = useState(initialView === "editor" && Boolean(initialPath));
  ...
  async function loadArticles() {
    ...
    try {
      ...
      if (!response.ok) setError(body.error || "Unable to load articles."); else setArticles(Array.isArray(body.articles) ? body.articles : []);
    } catch (reason) {
  ...
  useEffect(() => {
    ...
    if (saved && initialView === "editor" && !initialPath && !initialPathError) { try { setInput(normalizeDraftInput(JSON.parse(saved))); setNotice("Recovered a local unsaved draft."); setAutoSaveState("Saved locally"); } catch { window.localStorage.removeItem("gulshan-author-draft"); } }
    if (initialPathError) {
      setError(initialPathError);
    ...
    void (async () => {
      ...
      try {
        ...
        const body = await response.json().catch(() => ({}));
        if (response.ok && body.input && body.path) { setInput(normalizeDraftInput(body.input)); setPath(String(body.path)); setSha(String(body.sha || "")); setView("editor"); }
        else if (response.ok) setError("The repository returned an invalid article payload.");
        else setError(body.error || "Unable to open article.");
      } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to open article. Check your connection and try again."); }
      finally { setBusy(false); setEditorLoading(false); }
    })();
  }, [initialView, initialPath, initialPathError]);
  ...
  function update(field: keyof ArticleDraftInput, value: string | boolean) { setInput((current) => { const next = { ...current, [field]: value }; if (field === "difficulty") delete next.invalidDifficulty; return next; }); setValidation(null); setNotice(""); setError(""); }
  ...
  if (editorLoading && view === "editor") return <main className="author-workspace"><AuthorHeader /><section className="author-editor container" aria-busy="true"><div className="author-editor-loading"><div className="eyebrow">Private editor</div><h1>Loading article…</h1><p>Retrieving the requested article from the repository.</p><div className="editor-loading-grid" aria-hidden="true"><span /><span /><span /></div></div></section></main>;
  if (initialView === "editor" && initialPath && !path && error && !editorLoading) return <main className="author-workspace"><AuthorHeader /><section className="author-editor container"><div className="author-editor-error"><div className="eyebrow">Private editor</div><h1>Unable to open article</h1><p>The article could not be loaded from the repository.</p><p className="form-error" role="alert"><AlertCircle width={15} height={15} aria-hidden="true" /> {error}</p><div className="author-error-actions"><button type="button" className="author-quiet-button" onClick={() => window.location.reload()}>Retry</button><Link href="/author" className="btn btn-primary">Back to articles</Link></div></div></section></main>;
