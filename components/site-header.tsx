  ...
  return (
    <header className="site-header">
      <div className="container">
        ...
        {open && <nav id="mobile-navigation" className="mobile-nav" aria-label="Mobile navigation">{links.map((link) => <Link key={link.href} className="nav-link" href={link.href} aria-current={pathname.startsWith(link.href) ? "page" : undefined} onClick={() => setOpen(false)}>{link.label}</Link>)}<Link className="nav-link" href="/search" onClick={() => setOpen(false)}>Search</Link></nav>}
      </div>
    </header>
  );
}
