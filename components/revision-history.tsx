...
export function RevisionHistory({ path, currentInput, onRestored }: Props) {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      if (!response.ok) setError(body.error || "Revision history unavailable."); else setRevisions(Array.isArray(body.revisions) ? body.revisions : []);
    } catch { setError("Revision history unavailable."); }
  ...
  async function inspectRevision(revision: Revision) {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) setError(body.error || "Unable to inspect this revision."); else if (body.input) { setSelected(revision); setSelectedInput(normalizeDraftInput(body.input)); } else setError("Unable to inspect this revision.");
    } catch { setError("Unable to inspect this revision."); }
    finally { setBusy(false); }
  }
  ...
  async function restoreRevision() {
    ...
    try {
      ...
      if (!response.ok) setError(body.error || "Unable to restore this revision."); else if (body.input && body.path) { onRestored(normalizeDraftInput(body.input), String(body.path), String(body.sha || "")); setError(""); }
      else setError("Unable to restore this revision.");
    } catch { setError("Unable to restore this revision."); }
    finally { setBusy(false); }
  }

  if (!mounted) return <div className="author-panel revision-history"><div className="eyebrow">Revision history</div><h2>Previous saves</h2><p className="author-muted">Revision history is ready after the editor mounts.</p></div>;
