...
export function ArticleCard({ post, featured = false }: { post: Post; featured?: boolean }) {
  ...
  return <Link className={featured ? "feature-card" : "post-card"} href={`/blog/${post.slug}`}><div className="feature-art"><Image src={post.coverImage} alt="" fill sizes={featured ? "(max-width: 900px) 100vw, 52vw" : "(max-width: 900px) 100vw, 30vw"} className="cover-image" /><span>{category?.shortName || "Writing"}</span></div><div className="card-meta"><span className="category">{category?.name}</span><span>·</span><span>{formatDate(post.date)}</span><span>·</span><span>{post.readingTime}</span></div><h3>{post.title}</h3><p className="card-description">{post.description}</p><span className="card-arrow">Read the piece <ArrowUpRight size={15} /></span></Link>;
}
