...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><ReadingProgress /><article className="article-shell"><div className="container"><header className="article-header"><div className="eyebrow">{category?.name}</div><h1>{post.title}</h1><p className="article-dek">{post.description}</p><div className="article-meta"><span className="author-dot" aria-hidden="true" /><span>{post.author}</span><span>·</span><span>Published {formatDate(post.date)}</span><span>·</span><span>{post.readingTime}</span>{post.updatedAt && <><span>·</span><span>Updated {formatDate(post.updatedAt)}</span></>}</div></header><div className="article-cover"><Image src={post.coverImage} alt={`Editorial visual representing ${post.title}`} fill priority sizes="(max-width: 900px) 100vw, 1100px" className="cover-image" /><span className="article-cover-label">{category?.shortName} / field notes</span></div><div className="article-layout"><TableOfContents items={toc} /><div className="article-prose"><MDXRemote source={post.content} components={mdxComponents} options={{ blockJS: true, blockDangerousJS: true, mdxOptions: { remarkPlugins: [remarkGfm], rehypePlugins: [rehypeSlug, [rehypePrettyCode, { theme: "github-dark-dimmed" }]] } }} /><div className="article-footer"><div className="author-card"><div className="author-avatar">GK</div><div><h3>Gulshan Kumar</h3><p>3rd Year B.Tech Undergraduate in Information Technology. Writing about AI, software engineering, projects, and technology.</p><div className="author-links"><a href={siteConfig.links.github} target="_blank" rel="noreferrer">GitHub</a><a href="/about">About</a><a href={siteConfig.links.email}>Email</a></div></div></div><RelatedArticles recommendations={related} /><FeedbackReactions slug={post.slug} /></div></div></div></div></article><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /></>;
}
