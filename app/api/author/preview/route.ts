...
export async function POST(request: Request) {
  ...
  try {
    ...
    const previewPost: Post = { slug: input.slug || "preview", title: input.title || "Untitled article", description: input.description || "", date: input.date || new Date().toISOString().slice(0, 10), updatedAt: input.updatedAt || undefined, category: input.category, tags, author: "Gulshan Kumar", readingTime: input.readingTime || "", difficulty: input.difficulty || undefined, coverImage: input.coverImage || "/images/cover-editorial.svg", coverAlt: input.coverAlt, featured: Boolean(input.featured), status: input.status || "draft", content: input.content || "" };
